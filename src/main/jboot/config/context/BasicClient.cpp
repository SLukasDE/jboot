...
void BasicClient::install(boot::context::Context& context) const {
	if(refId.empty()) {
		...
		if(context.findObject<esl::com::basic::client::Interface::ConnectionFactory>(refId) == nullptr) {
			if(id.empty()) {
				...
				throw XMLException(*this, "Could not add basic client reference with id '" + id + "', because referenced object with id '" + refId + "' is not a basic client connection factory.");
			}
		}
	}
	context.addObject(id, create());
...
std::unique_ptr<esl::object::Interface::Object> BasicClient::create() const {
	...
	try {
		...
		throw XMLException(*this, "Could not create a basic client connection factory with id '" + id + "' for implementation '" + implementation + "' because an unknown exception occurred.");
	}
