...
void BasicServer::install(boot::context::Context& context) const {
	if(refId.empty()) {
		...
		if(context.findObject<esl::com::basic::server::Interface::Socket>(refId) == nullptr) {
			if(id.empty()) {
...
std::unique_ptr<esl::object::Interface::Object> BasicServer::create() const {
	std::vector<std::pair<std::string, std::string>> eslSettings;
	for(const auto& setting : settings) {
		eslSettings.push_back(std::make_pair(setting.key, evaluate(setting.value, setting.language)));
	}

	std::unique_ptr<esl::com::basic::server::Interface::Socket> socket;
	try {
		socket = esl::getModule().getInterface<esl::com::basic::server::Interface>(implementation).createSocket(eslSettings);
	}
